...
kable(word.total.region)


```
We can see here that there is a large discrepancy in the amount of text for each region. Some only have a single sentence of less than ten words, while others have over a thousands words of text.
...
#word.count.region$word <- factor(word.count.region$word, levels = c(word.count.region$word)

# View the structure of the data frame. I need word to be a factor so that it is ordered correctly in the next graphs.
#str(word.count.region)


  #mutate(word = reorder(word, rank))
  

# This has the words arranged alphabetically
...
# ggplot(word.count.region, aes(x = n, y = fct_reorder(word, n), fill = n)) +
#   geom_bar(stat = "identity", show.legend = FALSE) +
#   facet_wrap(~Region, ncol = 4, scales = "free_y") +
#   labs(title = "Most Frequently Used Words",
#        subtitle = "by Region")
# 
# ggplot(word.count.region, aes(x = n, y = reorder(word, n), fill = n)) +
#   geom_bar(stat = "identity", show.legend = FALSE) +
#   facet_wrap(~Region, ncol = 4, scales = "free_y") +
#   labs(title = "Most Frequently Used Words",
#        subtitle = "by Region")
...
# 
# 
# ggplot(word.count.region, aes(x = n, y = reorder(word, n))) +
#   geom_col(show.legend = FALSE) +
#   facet_wrap(~Region, ncol = 4, scales = "free_y") +
#   labs(title = "Most Frequently Used Words",
#        subtitle = "by Region")
...
frequency %>% 
  ...
  geom_col() +
  labs(title = "TRS Mentions by Region",
