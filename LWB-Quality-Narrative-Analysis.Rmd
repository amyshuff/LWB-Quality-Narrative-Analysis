...
narrative.nostops <- narrative.tidy %>% 
...
word.count.total <- narrative.nostops %>% 
...
# Now we'll do the same, but for each Board region
word.count.region <- narrative.nostops %>% 
  group_by(Board) %>% 
  count(word, sort = TRUE) %>% 
  pivot_wider(names_from = Board, values_from = n) %>% 
